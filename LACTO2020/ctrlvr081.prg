...
SCAN
		If FPAR+Q5<DATE()-231 
			...
			xCARGA(1)=IIF(UCAL>DATE()-231 And UCAL<=DATE()-210 And STAT='CARGA',xCARGA(1)+1,xCARGA(1))
			xINSEM(1)=IIF(UCAL>DATE()-231 And UCAL<=DATE()-210 And (STAT='CARGA' Or STAT='INSEM'),xINSEM(1)+1,xINSEM(1))
		Endif

		If FPAR+Q5<DATE()-210 
			nVACAS(2)=IIF(UCAL<DATE()-210 And (STAT='INSEM' Or STAT='CARGA'),nVACAS(2),nVACAS(2)+1)
			xCARGA(2)=IIF(UCAL>DATE()-210 And UCAL<=DATE()-189 And STAT='CARGA',xCARGA(2)+1,xCARGA(2))
			xINSEM(2)=IIF(UCAL>DATE()-231 And UCAL<=DATE()-210 And (STAT='CARGA' Or STAT='INSEM'),xINSEM(2)+1,xINSEM(2))
		Endif

		If FPAR+Q5<DATE()-189 
			nVACAS(3)=IIF(UCAL<DATE()-189 And (STAT='INSEM' Or STAT='CARGA'),nVACAS(3),nVACAS(3)+1)
			xCARGA(3)=IIF(UCAL>DATE()-189 And UCAL<=DATE()-168 And STAT='CARGA',xCARGA(3)+1,xCARGA(3))
			xINSEM(2)=IIF(UCAL>DATE()-231 And UCAL<=DATE()-210 And (STAT='CARGA' Or STAT='INSEM'),xINSEM(2)+1,xINSEM(2))
		Endif

		If FPAR+Q5<DATE()-168
			nVACAS(4)=IIF(UCAL<DATE()-168 And (STAT='INSEM' Or STAT='CARGA'),nVACAS(4),nVACAS(4)+1)
			xCARGA(4)=IIF(UCAL>DATE()-168 And UCAL<=DATE()-147 And STAT='CARGA',xCARGA(4)+1,xCARGA(4))
			xINSEM(2)=IIF(UCAL>DATE()-231 And UCAL<=DATE()-210 And (STAT='CARGA' Or STAT='INSEM'),xINSEM(2)+1,xINSEM(2))
		Endif

		If FPAR+Q5<DATE()-147
			nVACAS(5)=IIF(UCAL<DATE()-147 And (STAT='INSEM' Or STAT='CARGA'),nVACAS(5),nVACAS(5)+1)
			xCARGA(5)=IIF(UCAL>DATE()-147 And UCAL<=DATE()-126 And STAT='CARGA',xCARGA(5)+1,xCARGA(5))
			xINSEM(2)=IIF(UCAL>DATE()-231 And UCAL<=DATE()-210 And (STAT='CARGA' Or STAT='INSEM'),xINSEM(2)+1,xINSEM(2))
		Endif

		If FPAR+Q5<DATE()-126
			nVACAS(6)=IIF(UCAL<DATE()-126 And (STAT='INSEM' Or STAT='CARGA'),nVACAS(6),nVACAS(6)+1)
			xCARGA(6)=IIF(UCAL>DATE()-126 And UCAL<=DATE()-105 And STAT='CARGA',xCARGA(6)+1,xCARGA(6))
			xINSEM(2)=IIF(UCAL>DATE()-231 And UCAL<=DATE()-210 And (STAT='CARGA' Or STAT='INSEM'),xINSEM(2)+1,xINSEM(2))
		Endif

		If FPAR+Q5<DATE()-105
			nVACAS(7)=IIF(UCAL<DATE()-105 And (STAT='INSEM' Or STAT='CARGA'),nVACAS(7),nVACAS(7)+1)
			xCARGA(7)=IIF(UCAL>DATE()-105 And UCAL<=DATE()-84 And STAT='CARGA',xCARGA(7)+1,xCARGA(7))
			xINSEM(2)=IIF(UCAL>DATE()-231 And UCAL<=DATE()-210 And (STAT='CARGA' Or STAT='INSEM'),xINSEM(2)+1,xINSEM(2))
		Endif

		If FPAR+Q5<DATE()-84
			nVACAS(8)=IIF(UCAL<DATE()-84 And (STAT='INSEM' Or STAT='CARGA'),nVACAS(8),nVACAS(8)+1)
			xCARGA(8)=IIF(UCAL>DATE()-84 And UCAL<=DATE()-63 And STAT='CARGA',xCARGA(8)+1,xCARGA(8))
			xINSEM(2)=IIF(UCAL>DATE()-231 And UCAL<=DATE()-210 And (STAT='CARGA' Or STAT='INSEM'),xINSEM(2)+1,xINSEM(2))
		Endif

		If FPAR+Q5<DATE()-63
			nVACAS(9)=IIF(UCAL<DATE()-63 And (STAT='INSEM' Or STAT='CARGA'),nVACAS(9),nVACAS(9)+1)
			xCARGA(9)=IIF(UCAL>DATE()-63 And UCAL<=DATE()-42 And STAT='CARGA',xCARGA(9)+1,xCARGA(9))
			xINSEM(2)=IIF(UCAL>DATE()-231 And UCAL<=DATE()-210 And (STAT='CARGA' Or STAT='INSEM'),xINSEM(2)+1,xINSEM(2))
		Endif
		
		If FPAR+Q5<DATE()-42 
			nVACAS(10)=IIF(UCAL<DATE()-42 And (STAT='INSEM' Or STAT='CARGA'),nVACAS(10),nVACAS(10)+1)
			xCARGA(10)=IIF(UCAL>DATE()-42 And UCAL<=DATE()-21 And STAT='CARGA',xCARGA(10)+1,xCARGA(10))
			xINSEM(2)=IIF(UCAL>DATE()-231 And UCAL<=DATE()-210 And (STAT='CARGA' Or STAT='INSEM'),xINSEM(2)+1,xINSEM(2))
		Endif
...
Append Blank
Append Blank
Replace CONCEPTO With 'Total',COL1 With Str(xVACAS(15),5),COL2 With Str(xDETEC(15),4),COL3 With IIF(xDETEC(15)>0,Str(xDETEC(15)/xVACAS(15)*100,3),'  0'),COL4 With Str(nVACAS(15),5),COL6 With Str(xCARGA(15),4),COL7 With IIF(nVACAS(15)>0,Str(xCARGA(15)/xINSEM(15)*100,3),'  0'),COL8 With IIF(nVACAS(15)>0,Str(xCARGA(15)/nVACAS(15)*100,3),'  0')
*Browse
