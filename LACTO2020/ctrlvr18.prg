...
PROCEDURE RV1068
PARAMETER xCDIAS
Dimension xVACAS(30),xDETEC(30),nVACAS(30),xCARGA(30),xABTOS(30) 
xVACAS=0
xDETEC=0
nVACAS=0
xCARGA=0
...
SELECT CALOR
SET ORDER To 2
SET FILTER TO SER>0 &&OR CLAVE=9

SELECT REG
SET ORDER TO 2
SET FILTER TO NP>0 AND (FB2=B OR FB2>=DATE()-365) &&AND PSV#'S' 
...
** -----------------
* 14 -----------------------------
		If FPAR+Q5<=DATE()-273 AND (FB2=B OR FB2>=FPAR+Q5)
			xVACAS(14)=IIF(UCAL<=DATE()-273 And STAT#'FRESC' AND STAT#'CARGA',xVACAS(14)+1,xVACAS(14))

			Select CALOR
			Seek REG.ID
			Scan While CALOR.ID=REG.ID 
				xDETEC(14)=IIF(FECHA>=DATE()-294 And FECHA<=DATE()-273 And (CLAVE=4 Or CLAVE=5),xDETEC(14)+1,xDETEC(14))
			EndScan
			Select REG
		Endif
* 13 -----------------------------
		If FPAR+Q5<=DATE()-252 AND (FB2=B OR FB2>=FPAR+Q5)
			xVACAS(13)=IIF(UCAL<=DATE()-252 And STAT='CARGA',xVACAS(13),xVACAS(13)+1)

			Select CALOR
			Seek REG.ID
			Scan While CALOR.ID=REG.ID 
				xDETEC(13)=IIF(FECHA>=DATE()-273 And FECHA<=DATE()-252 And (CLAVE=4 Or CLAVE=5),xDETEC(13)+1,xDETEC(13))
			EndScan
			Select REG
		Endif
* 12 -----------------------------
		If FPAR+Q5<=DATE()-231 AND (FB2=B OR FB2>=FPAR+Q5)
		...
		Endif
* 11 ------------------------------
		If FPAR+xCDIAS<=DATE()-210 AND (FB2=B OR FB2>=FPAR+Q5) 
		...
		Endif
* 10 -----------------------------
		If FPAR+xCDIAS<=DATE()-189 AND (FB2=B OR FB2>=FPAR+Q5) 
		...
		Endif
* 9 ------------------------------
		If FPAR+xCDIAS<=DATE()-168 AND (FB2=B OR FB2>=FPAR+Q5) 
		...
		Endif
* 8 -------------------------------
		If FPAR+xCDIAS<=DATE()-147 AND (FB2=B OR FB2>=FPAR+Q5) 
		...
		Endif
* 7 ------------------------------
		If FPAR+xCDIAS<=DATE()-126 AND (FB2=B OR FB2>=FPAR+Q5) 
		...
		Endif
* 6 -------------------------------
		If FPAR+xCDIAS<=DATE()-105 AND (FB2=B OR FB2>=FPAR+Q5) 
		...
		Endif
* 5 -----------------------------
		If FPAR+xCDIAS<=DATE()-84 AND (FB2=B OR FB2>=FPAR+Q5) 
		...
		Endif
* 4 -----------------------------
		If FPAR+xCDIAS<=DATE()-63 AND (FB2=B OR FB2>=FPAR+Q5) 
		...
		Endif
* 3 -----------------------------		
		If FPAR+xCDIAS<=DATE()-42 AND (FB2=B OR FB2>=FPAR+Q5) 
		...
		Endif
* 2 -----------------------------		
		If FPAR+xCDIAS<=DATE()-21 AND (FB2=B OR FB2>=FPAR+Q5) 
...
* 1 -------------------------- 
		...
		If FPAR+xCDIAS<=DATE() AND (FB2=B OR FB2>=FPAR+Q5) 
			xVACAS(12)=IIF(UCAL<=DATE() AND STAT='CARGA',xVACAS(12),xVACAS(12)+1)   && CORRECTO
	...
	xVACAS(20)=xVACAS(1)+xVACAS(2)+xVACAS(3)+xVACAS(4)+xVACAS(5)+xVACAS(6)+xVACAS(7)+xVACAS(8)+xVACAS(9)+xVACAS(10)+xVACAS(11)+xVACAS(12)+xVACAS(13)+xVACAS(14)
	xDETEC(20)=xDETEC(1)+xDETEC(2)+xDETEC(3)+xDETEC(4)+xDETEC(5)+xDETEC(6)+xDETEC(7)+xDETEC(8)+xDETEC(9)+xDETEC(10)+xDETEC(11)+xDETEC(12)+xDETEC(13)+xDETEC(14)

ENDSCAN

SELECT REG
SET ORDER TO 2
SET FILTER TO NP>0 AND STAT#'SUCIA' AND (FB2=B OR FB2>=DATE()-365) AND PSV#'S' 
GO TOP

** CALCULO DE ELEGIBLES
** ---------------------------*
SCAN
* 14 -------------------------*
		IF FPAR+Q5<=DATE()-273 AND (FB2=B OR FB2>=FPAR+Q5) 
			nVACAS(14)=IIF(UCAL<=DATE()-273 And STAT='CARGA',nVACAS(14),nVACAS(14)+1)
		Endif
* 13 -------------------------*
		IF FPAR+Q5<=DATE()-252 AND (FB2=B OR FB2>=FPAR+Q5) 
			nVACAS(13)=IIF(UCAL<=DATE()-252 And STAT='CARGA',nVACAS(13),nVACAS(13)+1)
		Endif
* 12 -------------------------*
		IF FPAR+Q5<=DATE()-231 AND (FB2=B OR FB2>=FPAR+Q5) 
...
SCAN
* 14 -------------------------*
			xCARGA(14)=IIF(FECHA2>=DATE()-294 And FECHA2<=DATE()-273 And STAT='CARGA',xCARGA(14)+1,xCARGA(14))
* 13 -------------------------*
			xCARGA(13)=IIF(FECHA2>=DATE()-273 And FECHA2<=DATE()-252 And STAT='CARGA',xCARGA(13)+1,xCARGA(13))
* 12 -------------------------*
			xCARGA(1)=IIF(FECHA2>=DATE()-252 And FECHA2<=DATE()-231 And STAT='CARGA',xCARGA(1)+1,xCARGA(1))
* 11 -------------------------
			xCARGA(2)=IIF(FECHA2>=DATE()-231 And FECHA2<=DATE()-210 And STAT='CARGA',xCARGA(2)+1,xCARGA(2))
* 10 -------------------------
			xCARGA(3)=IIF(FECHA2>=DATE()-210 And FECHA2<=DATE()-189 And STAT='CARGA',xCARGA(3)+1,xCARGA(3))
* 9 --------------------------
			xCARGA(4)=IIF(FECHA2>=DATE()-189 And FECHA2<=DATE()-168 And STAT='CARGA',xCARGA(4)+1,xCARGA(4))
* 8 --------------------------
			xCARGA(5)=IIF(FECHA2>=DATE()-168 And FECHA2<=DATE()-147 And STAT='CARGA',xCARGA(5)+1,xCARGA(5))
* 7 --------------------------
			xCARGA(6)=IIF(FECHA2>=DATE()-147 And FECHA2<=DATE()-126 And STAT='CARGA',xCARGA(6)+1,xCARGA(6))
* 6 -------------------------- 
			xCARGA(7)=IIF(FECHA2>=DATE()-126 And FECHA2<=DATE()-105 And STAT='CARGA',xCARGA(7)+1,xCARGA(7))
* 5 -------------------------
			xCARGA(8)=IIF(FECHA2>=DATE()-105 And FECHA2<=DATE()-84 And STAT='CARGA',xCARGA(8)+1,xCARGA(8))
* 4 -------------------------
			xCARGA(9)=IIF(FECHA2>=DATE()-84 And FECHA2<=DATE()-63 And STAT='CARGA',xCARGA(9)+1,xCARGA(9))
* 3 -------------------------		
			xCARGA(10)=IIF(FECHA2>=DATE()-63 And FECHA2<=DATE()-42 And STAT='CARGA',xCARGA(10)+1,xCARGA(10))
* 2 -------------------------		
			xCARGA(11)=0
* 1 ----------------------
			xCARGA(12)=0
...
nVACAS(20)=nVACAS(1)+nVACAS(2)+nVACAS(3)+nVACAS(4)+nVACAS(5)+nVACAS(6)+nVACAS(7)+nVACAS(8)+nVACAS(9)+nVACAS(10)+nVACAS(11)+nVACAS(12)+nVACAS(13)+nVACAS(14)
xCARGA(20)=xCARGA(1)+xCARGA(2)+xCARGA(3)+xCARGA(4)+xCARGA(5)+xCARGA(6)+xCARGA(7)+xCARGA(8)+xCARGA(9)+xCARGA(10)+xCARGA(11)+xCARGA(12)+xCARGA(13)+xCARGA(14)

SELECT REG
SET FILTER TO
Create Cursor REPORTE (Concepto c(20),COL1 c(5),COL2 c(5),COL3 c(5),COL4 c(5),COL5 c(5),COL6 c(5))
Append Blank
Replace CONCEPTO With DTOC(DATE()-294),COL1 With Str(xVACAS(14),4),COL2 With Str(xDETEC(14),4),COL3 With Str(xDETEC(14)/xVACAS(14)*100,3),COL4 With Str(nVACAS(14),4),COL5 With Str(xCARGA(14),4),COL6 With Str(xCARGA(14)/nVACAS(14)*100,3)
Append Blank
Replace CONCEPTO With DTOC(DATE()-273),COL1 With Str(xVACAS(13),4),COL2 With Str(xDETEC(13),4),COL3 With Str(xDETEC(13)/xVACAS(13)*100,3),COL4 With Str(nVACAS(13),4),COL5 With Str(xCARGA(13),4),COL6 With Str(xCARGA(13)/nVACAS(13)*100,3)
Append Blank
...
Append Blank
Replace CONCEPTO With DTOC(DATE()-147),COL1 With Str(xVACAS(6),4),COL2 With Str(xDETEC(6),4),COL3 With Str(xDETEC(6)/xVACAS(6)*100,3),COL4 With Str(nVACAS(6),4),COL5 With Str(xCARGA(6),4),COL6 With Str(xCARGA(6)/nVACAS(6)*100,3)
Append Blank
...
Append Blank
Append Blank
Replace CONCEPTO With 'Total',COL1 With Str(xVACAS(20),5),COL2 With Str(xDETEC(20),4),COL3 With IIF(xDETEC(20)>0,Str(xDETEC(20)/xVACAS(20)*100,3),'  0'),COL4 With Str(nVACAS(20),5),COL5 With Str(xCARGA(20),4),COL6 With IIF(nVACAS(20)>0,Str(xCARGA(20)/nVACAS(20)*100,3),'  0')
